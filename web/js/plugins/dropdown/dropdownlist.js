	...
	$.fn.dropdownlist=function(opts){
		...
		return this.each(function(){
			...
			var select = $('<div>',
							{'class':(settings.disabled?settings.disabledclass:settings.selectclass),
							 ...
							 'click':function(evt){
							 	...
							 	$('div.'+settings.listboxclass,$this).css({'top':218,'left':211}).show();
							 	stopBubble(evt);
							 }
							}).appendTo($this);
			...
			var listbox = $('<div>',{
							...
							'css': {'position':'absolute','width':220,'overflow':'hidden','overflow-y':'auto','display':'none','z-index':1000}
						}).appendTo($this);
			if(settings.checkbox)
			{
				var tfoot = $('<tfoot><td colspan="'+settings.columns+'"><span class="btn-area"><input type="button" value="确定" class="btn-ok"><input type="button" value="取消" class="btn-cancel"></span></td></tfoot>');
