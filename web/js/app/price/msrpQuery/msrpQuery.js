...
$(function() {
	...
	/*** 数据范围change事件*/
		$("#msrpType").on("change",function(){
			$('.subModelLIContainer .subModelModalResultContainer').html("");
			if($("#msrpType").val() == 1){
				$("#startDate").val(endDate);
				$("#startDate-container.input-append.date").datetimepicker("setStartDate", endDate);
				$("#endDate").val(endDate);
				$("#endDate-container.input-append.date").datetimepicker("setStartDate", endDate);
			} else{
				$("#startDate").val(endDate.substr(0,4)+"-01");
				$("#startDate-container.input-append.date").datetimepicker("setStartDate", beginDate);
				$("#endDate").val(endDate);
				$("#endDate-container.input-append.date").datetimepicker("setStartDate", beginDate);
			}
		});
	/*** 数据范围change事件结束*/
	/**获取时间模板*/
	function getDateTp(type){
		...
		htmlStr += '	  		<input type="text" value="' + endDate + '"  readonly="readonly" class="input-mini white" placeholder="开始日期" id="startDate"/>';
		...
		htmlStr += '		<h6  style="display:inline"> <font color="red">提示:车型全选历史指导价时因数据量过大建议查询时间段不要超过三个月。</font></h6>';
		htmlStr += '		</span>';*/
		htmlStr += '	</div>';
	 	htmlStr += '</li>';
	 	return htmlStr;
	...
	$('#bodyTypeModal').on('show', function (e) {
		...
		$('#bodyTypeModalBody').load(ctx+"/price/msrpQuery/getBodyType",getParams(),function(){
			//弹出框设置默认选中项结果集		
	...
	$("#subModelModal").on("show", function (e) {
		...
		$("#subModelModalBody").load(ctx+"/price/msrpQuery/getSubmodelModal",getParams(),function(){
			//弹出框设置默认选中项结果集		
...
//					//判断是不是隐藏
	...
	$('#autoVersionModal').on('show', function (e) {	
		...
		$('#autoVersionModalBody').load(ctx+"/price/msrpQuery/getAutoCustomGroup",getParams(),function(){
			//设置默认选中项
	...
	$("#queryBtn").on("click", function (e) {
		...
		$.ajax({
	        type: "POST",
			url: ctx+"/price/msrpQuery/getMsrpQueryData",
			data: getParams(),
...
//	    return Math.round(num*vv)/vv;
	...
	function showData(data){
		...
		{	
			...
			tbodyHtml += "<tr>";
			if($("#msrpType").val() == 1){
				tbodyHtml += "<td class='tHead' style=' text-align:center; border-right:1px solid white' rowspan='2'>" + "年月" + "</td>";
			}
			...
			if($("#msrpType").val() == 1){
				...
				tbodyHtml += "<td class='tHead' style=' text-align:center; border-bottom:1px solid white; border-right:1px solid white;' colspan='" + num + "' >" + "厂商指导价(元)" + "</td>";
			}
			tbodyHtml += "</tr>";
			...
			for(var i = startNumber;i < endNumber; i++){
				tbodyHtml += "<tr>";
				if($("#msrpType").val() == 1){
					tbodyHtml += "<td class='tdSubModel'>" + data.totalData[i].yearMonth + "</td>";
				}
				...
				if($("#msrpType").val() == 1){
					...
					for(var k = 0; k < data.monthData.length; k++){
						if(data.totalData[i].vid == data.monthData[k].vid && data.monthData[k].yearMonth == ym){
							tbodyHtml += "<td class='tdSubModel'>" + formatNum(data.monthData[k].msrp) + "</td>";
						}
					}
				for(var k = 0; k < data.monthData.length; k++){
						if(data.totalData[i].vid == data.monthData[k].vid && data.monthData[k].yearMonth != ym){
...
window.getParams = function()
{
	...
	var msrpType = $("#msrpType").val();//0为显示历史指导价，1为最新指导价；
	//数据分析类型默认为销量
...
window.showSubModel = function(type)
{
	...
	/*根据选择条件隐藏结束*/
	/* 车型全选框勾选判断 */
	//alert($("#selectorResultContainerBySubModel").val() == "")
	//console.log("====="+($("#selectorResultContainerBySubModel ul").html() == "")+"   "+$("#selectorResultContainerBySubModel ul").html()	)
	//清空本页的选项
	//把结果集里选中的都打钩	
...
/*	if($("#choseType").val() != 1){
		...
		if($("#choseType").val() == 3){
			if(!$("#tabs-brand").html() == ""){
				...
				if($("#selectorResultContainerBySubModel .removeBtnByResult").length == num ){
					$("#modelAll").prop("checked",true);
				}
				//如果全选框打钩，全选的情况下全选
				if($("#myModalLabel #modelAll").prop("checked") && $("#selectorResultContainerBySubModel .removeBtnByResult").length == num ){
					$("#tabs-brand .selectorContentTd").find("input").each(function(){
						  var ths = $(this);
						  if(ths.parent().css("display") != "none"){
			...
			if(!$("#tabs-segment").html() == ""){
				...
				//如果全选框打钩，全选的情况下全选
				if($("#myModalLabel #modelAll").prop("checked")  && $("#selectorResultContainerBySubModel .removeBtnByResult").length == num && num > 0){
					$("#tabs-segment .selectorContentTd").find("input").each(function(){
						var ths = $(this);
						  if(ths.parent().css("display") != "none"){
							  ths.prop("checked",true);
		...
		$("#tabs-competingProducts .selectorContentTd").find("input").each(function(){
			   var ths = $(this);
				//判断是不是隐藏
			   if(ths.parent().css("display") != "none"){
					num++;
		...
		if(  $("#selectorResultContainerBySubModel .removeBtnByResult").length == num && num > 0){
			$("#modelAll").prop("checked",true);
		}
		//如果全选框打钩，全选的情况下全选
		if($("#myModalLabel #modelAll").prop("checked") &&  $("#selectorResultContainerBySubModel .removeBtnByResult").length > 500 && num > 0){
			$("#tabs-competingProducts .selectorContentTd").find("input").each(function(){
				var ths = $(this);
				  if(ths.parent().css("display") != "none"){
					  ths.prop("checked",true);
...
function getModelPage(id,type,inputType,timeType)
{
	...
	if(!$.trim($('#' + id).html())){
		...
		//触发请求
		showLoading(id);
		$("#" + id).load(ctx+"/price/msrpQuery/getSubmodelModal",params,function(){
			var choseType = $("#choseType").val(); 
			//把结果集里选中的都打钩	
			if(choseType ==1){
				var ty = $("#tabs-competingProducts .subModelIdInput");
