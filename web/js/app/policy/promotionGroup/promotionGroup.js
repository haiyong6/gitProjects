...
var subsidyTypeArray = new Array(TCZC,LSZC,RYJL,JRDK,ZHZC,ZSBX,ZSLP,XZCX);
...
//				}
	 ...
	 */
	 $('#objectType').on('change', function () {
		 $("#chartId").html("");//清空图
		$("#chartTable").html("");//清空表
		$("#gridTbody").html("");//清空右边数据
		 //当页面条件是非型号时显示正斜对应选择项
		/* if($("select[id='objectType'] option:selected").val()!='1'){
				$("#duiyingDiv").attr("style","display:block")
			}else{
	...
	$('#analysisContentType').on('change', function () {
		...
		$.ajax({
			   type: "POST",
			   url: ctx+"/promotionGroup/resetDate",
	...
	{	
		...
		$.ajax({
			   type: "POST",
			   url: ctx+"/promotionGroup/checkPopBoxData",
			   data: paramsObj,
			   dataType:'json',
			   success: function(data){
				   if(data)
...
//			} 
		...
		//var segmentIds =  $("#segmentModalResultContainer li div").map(function(){return $.trim($(this).attr("value"))+"~"+$.trim($(this).find("input").eq(0).attr("leveltype"));}).get().join(",");
...
//		span2Text = "Dealer "+objectNameEn + dataNameEn+" trend" + span2Text;
	...
	function showVersionGrid(grid)
	{
		...
		if(grid)
		{
			for(var i = 0; i < grid.length; i++)
			{
				...
				tbodyHtml += "<td style='width:7%;text-align: center;' class='text-right "+className+"'>" + formatData(dataObj.c5) + "</td>";
				tbodyHtml += "<td style='width:8%;text-align: center;' class='text-right "+className+"'>" + formatData(dataObj.c6) + "</td>";
				tbodyHtml += "<td style='width:8%;text-align: center;' class='text-right "+className+"'>" + formatData(dataObj.c7) + "</td>";
	...
	$('#queryBtn').on('click', function (e) {
		...
		if(paramsValidate2(params)) return;
		if(isPromotion){priceType = 1;};
		...
		$.ajax({
			   type: "POST",
			   url: ctx+"/promotionGroup/loadChartAndTable",
			   data: params,
			   success: function(data){
					 ...
					 if(!data.chart.showType){
						...
						$("#showTable").click();
						 ...
						 $("#priceTypeDiv").css("display","block");
						 	...
						 	showChartByType(data.chart);//画图表
							CreateChartTableByPromotion(data.chart,params);//画时间列表
							$("h4[class='wushuju']").css("display","none");
					 }
		...
		if(objectType == 1){
			...
			for (var i=0; i< grid.length; i++) {
				...
				if(trShow.indexOf(temp) == -1){
					...
					if(grid[i].objectName.replace(/\s+/g,"")==""){st="display:none;"}else{st="display:in-line;"};
					tbodyHtml += "<tr style='"+st+"' >";
...
function paramsValidate2(params){
	var objectType = params.objectType;
...
function getPromotionModelPage(id,type,inputType,timeType)
{
	...
	if(!$.trim($('#' + id).html())){
		...
		//触发请求
		showLoading(id);
		$('#' + id).load(ctx+"/policy/promotionGroup/getSubmodelModal",params,function(){
