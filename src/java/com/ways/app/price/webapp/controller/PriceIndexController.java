...
public class PriceIndexController extends BaseController {
	...
	protected final Log log = LogFactory.getLog(PriceIndexController.class);
	@Autowired
	private IPriceIndexManager manager;
	@Autowired
	 ...
	 * @throws Exception
	 */
    @RequestMapping("/priceIndex/priceIndexMain")
    public ModelAndView priceIndexOfMain(HttpServletRequest request) throws Exception
    {
        ...
        request.setAttribute("menuId", "005");//设置模块ID，用来填充菜单栏选中样式
        return new ModelAndView("/price/priceIndex/priceIndex", model.asMap());
    }
	 ...
	 * @throws Exception
	 */
    @RequestMapping("/priceIndex/tpDate")
     ...
     * @param response
     */
    @RequestMapping("/price/priceIndex/getPriceIndexAnalyseData")
    public void getPriceIndexAnalyseData(HttpServletRequest request, HttpServletResponse response)
    {
    	Map<String, Object> paramsMap = getPageParams(request);
    	String json = manager.getPriceIndexAnalyseData(request, paramsMap);
    	logManager.addModuleLog(request, paramsMap);
    	try {
			print(response,json);
		} catch (Exception e) {
     ...
     * @throws Exception
     */
    @RequestMapping("/price/priceIndex/getSubmodelModal")
	 ...
	 * @param response
	 */
	@RequestMapping("/exportPriceIndexOriginalData")
	public void exportPriceIndexOriginalData(HttpServletRequest request ,HttpServletResponse response)
	{
		...
		try {
	        ...
	        String excelName = java.net.URLEncoder.encode("价格监测-价格降幅"+ DateUtil.getCurrentTime(), "UTF-8");
