...
@Controller
public class ModelSalesAmountFawController extends BaseController {
	
	protected final Log log = LogFactory.getLog(ModelSalesAmountFawController.class);
	@Autowired
	private ModelSalesAmountFawManager modelSalesAmountFawManager;
	@Autowired
	private IModuleLogManager logManager;
	
	private String moduleName = "销售比例查询";
	private String moduleNameEn = "ModelSalesAmountFawQuery";
	 ...
	 * @throws Exception
	 */
    @RequestMapping("/salesQuerys/modelSalesAmountFawMain")
    public ModelAndView terminalAnalysisMain(HttpServletRequest request) throws Exception
    {
        Model model = new ExtendedModelMap();
        modelSalesAmountFawManager.initDate(request, getPageParams(request));
        request.setAttribute("menuId", "019");//设置模块ID，用来填充菜单栏选中样式
        request.setAttribute("moduleCode", moduleNameEn);
        return new ModelAndView("/salesQuery/modelSalesAmountFaw/modelSalesAmountFawMain", model.asMap());
    }
     ...
     * @throws Exception
     */
    @RequestMapping("/salesQuery/modelSalesAmountFaw/getSubmodelModal")
    public ModelAndView getSubmodelModal(HttpServletRequest request,HttpServletResponse response) throws Exception
    {
    	...
    	modelSalesAmountFawManager.getSubmodelModal(request, map);
     ...
     * @throws Exception
     */
	@RequestMapping("/salesQuery/modelSalesAmountFaw/getBodyType")
    public ModelAndView getBodyType(HttpServletRequest request) throws Exception 
    {
    	Model model = new ExtendedModelMap();
    	modelSalesAmountFawManager.getBodyType(request, getPageParams(request));
     ...
     * @throws Exception
     */
    @RequestMapping("/salesQuery/modelSalesAmountFaw/getAutoCustomGroup")
    ...
    {	
    	...
    	modelSalesAmountFawManager.getAutoCustomGroup(request, paraMap);
	 ...
	 * @param response
	 */
    @RequestMapping("/salesQuery/modelSalesAmountFaw/getModelSalesAmountFawData")
	public void getModelSalesAmountFawData(HttpServletRequest request,HttpServletResponse response)
	{
		...
		String json = modelSalesAmountFawManager.getModelSalesAmountFawData(request, paramsMap);
    ...
    public Map<String, Object> getPageParams(HttpServletRequest request)
	{
		...
		map.put("salesType", request.getParameter("salesType"));//0为显示销量，1为隐藏销量
		map.put("totalType", request.getParameter("totalType"));//0为显示累计，1为隐藏累计
