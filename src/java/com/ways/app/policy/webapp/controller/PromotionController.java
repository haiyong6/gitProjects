...
public class PromotionController extends BaseController{
	...
	private String moduleName = "促销走势分析";
	 ...
	 * @throws Exception
	 */
    @RequestMapping("/promotion/promotionMain")
    public ModelAndView promotionMain(HttpServletRequest request) throws Exception
    {
        Model model = new ExtendedModelMap();
        Map<String, Object> map = getPageParams(request);
        promotionManager.initDate(request, map);
        request.setAttribute("menuId", "010");//设置模块ID，用来填充菜单栏选中样式
        return new ModelAndView("/policy/promotion/promotionMain", model.asMap());
    }
	 ...
	 * @param response
	 */
	@RequestMapping(value="/promotion/resetDate")
	public void resetDate(HttpServletRequest request, HttpServletResponse response)
	{
		...
		String json = promotionManager.initDate(request, paramsMap);
		try {
			print(response, json);
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 * @param response
	 */
	@RequestMapping(value="/promotion/checkPopBoxData")
	public void checkPopBoxData(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if("1".equals(paramsMap.get("objectType")) || "2".equals(paramsMap.get("objectType"))) {
			//对象类型为车型或者型号时弹出框校验
			json = promotionManager.checkPopBoxData(request, paramsMap);
		} else {
			//对象类型为生产商时弹出框校验
			json = promotionManager.checkManfPopBoxData(request, paramsMap);
		}
		try {
			print(response, json);
		} catch (IOException e) {
     ...
     * @throws Exception
     */
    @RequestMapping("/policy/promotion/getSubmodelModal")
	 ...
	 * @param response
	 */
	@RequestMapping(value="/promotion/loadChartAndTable")
	public void loadPromotionChartAndTable(HttpServletRequest request, HttpServletResponse response)
	{
		...
		String json = promotionManager.loadChartAndTable(request, paramsMap);
	 ...
	 * @param response
	 */
	@RequestMapping("/promotion/exportExcel")
	public void exportExcel(HttpServletRequest request, HttpServletResponse response)
	{
		Map<String, Object> paramsMap = getPageParams(request);
		try {
			//Terminal-PromotionTrendAnalysis
			Workbook wb = promotionManager.exportExcel(request, paramsMap);
			String excelName = "";
			if("EN".equals(paramsMap.get("languageType"))){
				excelName = java.net.URLEncoder.encode("Terminal-PromotionTrendAnalysis"+ DateUtil.getCurrentTime(), "UTF-8");
			}else{
				excelName = java.net.URLEncoder.encode(mName+"-"+moduleName+ DateUtil.getCurrentTime(), "UTF-8");
			}
