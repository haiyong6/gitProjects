...
@Controller
public class SaleIncentiveQueryController extends BaseController {
	
	protected final Log log = LogFactory.getLog(SaleIncentiveQueryController.class);
	@Autowired
	private SaleIncentiveQueryManager saleIncentiveQueryManager;
	@Autowired
	private IModuleLogManager logManager;
	
	private String moduleName = "销售激励查询";
	 ...
	 * @throws Exception
	 */
    @RequestMapping("/policy/saleIncentiveQueryMain")
    public ModelAndView terminalAnalysisMain(HttpServletRequest request) throws Exception
    {
        Model model = new ExtendedModelMap();
        saleIncentiveQueryManager.initDate(request, getPageParams(request));
        ...
        return new ModelAndView("/policy/saleIncentiveQuery/saleIncentiveQueryMain", model.asMap());
    }
     ...
     * @throws Exception
     */
    @RequestMapping("/policy/saleIncentiveQuery/getSubmodelModal")
    public ModelAndView getSubmodelModal(HttpServletRequest request,HttpServletResponse response) throws Exception
    {
    	...
    	saleIncentiveQueryManager.getSubmodelModal(request, map);
     ...
     * @throws Exception
     */
	@RequestMapping("/policy/saleIncentiveQuery/getBodyType")
    public ModelAndView getBodyType(HttpServletRequest request) throws Exception 
    {
    	Model model = new ExtendedModelMap();
    	saleIncentiveQueryManager.getBodyType(request, getPageParams(request));
     ...
     * @throws Exception
     */
    @RequestMapping("/policy/saleIncentiveQuery/getAutoCustomGroup")
    ...
    {	
    	...
    	saleIncentiveQueryManager.getAutoCustomGroup(request, paraMap);
	 ...
	 * @param response
	 */
    @RequestMapping("/policy/saleIncentiveQuery/getSaleIncentiveQueryData")
	public void getSaleIncentiveQueryData(HttpServletRequest request,HttpServletResponse response)
	{
		Map<String, Object> paramsMap = getPageParams(request);
		DateUtil.getTPMix_Date(paramsMap);
		String json = saleIncentiveQueryManager.getSaleIncentiveQueryData(request, paramsMap);
     ...
     * @param response
     */
    @RequestMapping("/policy/saleIncentiveQuery/exportExcelMap")
    public void exportExcel(HttpServletRequest request,HttpServletResponse response){

		Map<String, Object> paramsMap = getPageParams(request);
		try {
			Workbook wb = saleIncentiveQueryManager.exportExcel(request, paramsMap);
			String excelName = java.net.URLEncoder.encode(moduleName, "UTF-8");
